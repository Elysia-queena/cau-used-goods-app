...
function getPeerAccountStatus(item) {
  const peerId = getPeerId(item)
  const buyerId = getBuyerId(item)
  const sellerId = getSellerId(item)
  if (sameId(peerId, buyerId)) {
    return accountStatusOf(item.buyer || { accountStatus: item.buyerAccountStatus || item.buyerStatus || item.buyer_account_status })
  }
  if (sameId(peerId, sellerId)) {
    return accountStatusOf(item.seller || item.product?.seller || { accountStatus: item.sellerAccountStatus || item.sellerStatus || item.seller_account_status })
  }
  return accountStatusOf(item.targetUser || item.otherUser || item.user || { accountStatus: item.targetAccountStatus || item.otherUserStatus || item.userStatus })
}

function profileAccountStatus(profile) {
  return accountStatusOf(profile?.user || profile || {})
}

function blockBannedPeer(status) {
  if (!isBannedUserStatus(status)) return false
  uni.showToast({ title: '该用户已被封禁', icon: 'none' })
  return true
}
  ...
  try {
    ...
    conversations.value = await Promise.all(conversations.value.map(async (item) => {
      ...
      return {
        ...
        targetAccountStatus: profileAccountStatus(profile) || item.targetAccountStatus || getPeerAccountStatus(item),
        avatar
      }
    }))
    updateTabBadge()
...
async function openChat(item) {
  ...
  let targetAccountStatus = item.targetAccountStatus || getPeerAccountStatus(item)
  if (blockBannedPeer(targetAccountStatus)) return
  if (targetUserId && (!targetNickname || !targetAvatar || !targetAccountStatus)) {
    const profile = await getPublicProfile(targetUserId).catch(() => null)
    if (profile) {
      targetAccountStatus = profileAccountStatus(profile) || targetAccountStatus
      if (blockBannedPeer(targetAccountStatus)) return
