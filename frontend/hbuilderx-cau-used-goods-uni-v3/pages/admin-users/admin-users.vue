  ...
  <view class="page">
    ...
    <view v-for="item in users" :key="item.id" class="card">
      ...
      <view class="actions">
        ...
        <button v-if="canRestoreUser(item)" class="mini ok" @click="changeStatus(item, 'NORMAL')">恢复</button>
      </view>
...
const currentRole = computed(() => {
  ...
  if (devRole === 'SUPER_ADMIN') return 'SUPER_ADMIN'
  const user = getUser() || {}
...
function canRestoreUser(item) {
  if (!item || item.accountStatus === 'NORMAL') return false
...
function changeStatus(item, status) {
  if (status === 'NORMAL' && item.accountStatus === 'BANNED' && !isSuperAdmin.value) {
    uni.showToast({ title: '只有超级管理员可以恢复封禁账号', icon: 'none' })
    return
  }
  ...
  uni.showModal({
    ...
    success: async (res) => {
      ...
      try {
        await updateAdminUserStatus(item.id, {
          accountStatus: status,
          reason,
          operatorRole: isSuperAdmin.value ? 'SUPER_ADMIN' : 'ADMIN'
        })
        uni.showToast({ title: '操作成功', icon: 'success' })
        loadUsers()
      } catch (error) {
        const fallback = status === 'NORMAL' && item.accountStatus === 'BANNED'
          ? '恢复封禁账号失败，请确认当前账号是超级管理员'
          : '操作失败'
