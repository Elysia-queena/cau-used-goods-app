...
const userRole = computed(() => String(user.value?.role || user.value?.userRole || user.value?.user_role || '').toUpperCase())
const isAdmin = computed(() => userRole.value === 'ADMIN' || userRole.value === 'SUPER_ADMIN')
const identityText = computed(() => userRole.value === 'SUPER_ADMIN' ? '\u8d85\u7ea7\u7ba1\u7406\u5458' : isAdmin.value ? '\u7ba1\u7406\u5458' : `\u5b66\u751f\u8ba4\u8bc1\uff1a${authText.value}`)
...
onShow(async () => {
  try {
    ...
    user.value = { ...cached, ...current, role: cached.role === 'SUPER_ADMIN' ? 'SUPER_ADMIN' : current.role }
    setUser(user.value)
  } catch (error) {
    if (error.message) {
